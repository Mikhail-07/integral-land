...
import TextBlock from "./TopView/TextBlock"
import TextBlock2 from "./TopView/TextBlock2"
import TextBlock3 from "./TopView/TextBlock3"
import TextBlock4 from "./TopView/TextBlock4"
import ConnectorLine from "./TopView/ConnectorLine"
import ImagePoint from "./TopView/ImagePoint"
import ImageArrow from "./TopView/ImageArrow"
import TextBlock5 from "./TopView/TextBlock5"
import TextBlock6 from "./TopView/TextBlock6"
...
const PhaseScroller = () => {
  ...
  const getScaleForPhase = (index) => {
    ...
    // Phase 0: half submerged below bottom, make image height = 2*vh at base width 100vw
    if (index === 0) {
      const baseHeightAt100vw = (natH / natW) * vw
      const needed = (2 * vh) / baseHeightAt100vw
      return Math.max(1, needed)
    }

    // Phases 1 & 2: half submerged to the right, make total width = 2*vw so visible half fills screen
    if (index === 1 || index === 2) {
      return 2
    }
  ...
  const initializeAnimation = () => {
    ...
    gsap.set([arrowRef1.current, arrowRef2.current], {
      opacity: 0,
      scale: 0.85,
    })
    gsap.set([textRef5.current, textRef6.current], {
      opacity: 0,
      y: 16,
    })
    const updateArrowTextPositions = () => {
      if (!sectionRef.current) {
        return
      }
      ...
      if (arrowRef2.current && textRef6.current) {
        ...
        gsap.set(textRef6.current, {
          position: "absolute",
          left: a2.left - sectionBox.left - 220,
          top: a2.bottom - sectionBox.top + 20,
          width: 200,
        })
      }
    }
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        onUpdate: (self) => {
          ...
          if (nearestIndex === 3) {
            gsap.to([arrowRef1.current, arrowRef2.current], {
              opacity: 0.85,
              scale: 1,
              duration: 0.6,
              ease: "power3.out",
            ...
            gsap.to([textRef5.current, textRef6.current], {
              opacity: 1,
              y: 0,
              duration: 0.8,
              ease: "power3.out",
  ...
  return (
    ...
    >
      <div className="relative h-full w-full px-6 py-16">
        ...
        <div className="pointer-events-none absolute left-10 top-24 z-10">
          <div
            ref={(node) => {
        ...
        <div className="pointer-events-none absolute left-10 bottom-24 z-10">
          <div
            ref={(node) => {
      ...
      <ImageArrow
        ...
        src={arrowDownSvg}
      />
      <div ref={textRef5} className="absolute z-10 pointer-events-none">
        <TextBlock5 />
      </div>
      <div ref={textRef6} className="absolute z-10 pointer-events-none">
        <TextBlock6 />
      </div>
    </section>
  )
